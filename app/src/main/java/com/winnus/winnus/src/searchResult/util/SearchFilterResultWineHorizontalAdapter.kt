...
class SearchFilterResultWineHorizontalAdapter(private val context: Context, private val items: List<FilteringRe>, val viewInterface: SearchResultActivityView): RecyclerView.Adapter<SearchFilterResultWineHorizontalAdapter.ViewHolder>() {
    ...
    class ViewHolder(private var binding: RecyclerItemWineSearchBinding) : RecyclerView.ViewHolder(binding.root){
        var bookMark = false
        fun bind(context: Context, item: FilteringRe, viewInterface: SearchResultActivityView, pos : Int){
            Glide.with(context)
                .load(item.wineImg)
            ...
            if(bookMark){
                binding.imgHeart.setImageResource(R.drawable.ic_winnus_haert_fill_20)
            }else{
                binding.imgHeart.setImageResource(R.drawable.ic_winnus_haert_solid_white_20)
            }
