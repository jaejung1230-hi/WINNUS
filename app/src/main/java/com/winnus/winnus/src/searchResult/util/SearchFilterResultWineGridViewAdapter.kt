...
class SearchFilterResultWineGridViewAdapter(private var context: Context, val items: List<FilteringRe>, val viewInterface : SearchResultActivityView): BaseAdapter() {
    var mMessageList = arrayListOf<FilteringRe>()

    init {
        mMessageList = items as ArrayList<FilteringRe>
    }

    fun updateList(data: List<FilteringRe>) {
        mMessageList = data as ArrayList<FilteringRe>
