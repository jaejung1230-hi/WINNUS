...
class ShopWinesGridRecyclerAdapter(private var context: Context, val items: List<Wine>, val viewInterface : DetailShopActivityView): RecyclerView.Adapter<ShopWinesGridRecyclerAdapter.ViewHolder>() {
    ...
    class ViewHolder(private var binding: GridviewItemWineSearchBinding) : RecyclerView.ViewHolder(binding.root){
        var bookMark = false
        fun bind(context: Context, item: Wine, viewInterface: DetailShopActivityView, pos : Int){
            Glide.with(context)
                .load(item.wineImg)
