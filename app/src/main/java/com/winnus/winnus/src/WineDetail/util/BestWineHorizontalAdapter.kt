...
class BestWineHorizontalAdapter(private val context: Context, private val items: List<BestWineListByType>): RecyclerView.Adapter<BestWineHorizontalAdapter.ViewHolder>() {
    ...
    class ViewHolder(private var binding: RecyclerItemWineForDetailBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(context: Context, item: BestWineListByType){
            Glide.with(context)
                .load(item.wineImg)
