...
class SearchFilterWineGridRecyclerAdapter(private var context: Context, val items: ArrayList<FilteringRe>, val viewInterface : SearchResultActivityView): RecyclerView.Adapter<SearchFilterWineGridRecyclerAdapter.ViewHolder>() {

    fun add(addlist: ArrayList<FilteringRe>){
        for(add in addlist){
            items.add(add)
        }
